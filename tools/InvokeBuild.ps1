...
task BuildModule {
    ...
    $extManifestPath = [Path]::Combine(
        $Manifest.ReleasePath,
        "$($Manifest.Module.Name).Extension",
        "$($Manifest.Module.Name).Extension.psd1")
    ...
    $extManifest = $extManifest.Replace(
        ...
        "CompanyName = ''",
        "CompanyName = '$($Manifest.Module.CompanyName)'"
    ).Replace(
        "Copyright = ''",
        "Copyright = '$($Manifest.Module.Copyright)'"
    )
    if ($Manifest.Module.ExportedCmdlets.Count) {
        $extManifest = $extManifest.Replace(
            "CmdletsToExport = @(",
            "CmdletsToExport = @(`n        '$($Manifest.Module.ExportedCmdlets.Keys -join "'`n        '")'"
        )
    }
